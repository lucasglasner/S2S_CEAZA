...
for p in reforecasts:
    print(p)
    if not os.path.isfile(p+'/merged_SST.nc'):

        data = []
        for r in glob(p+'/*SST.nc'):
            dd = xr.open_dataset(r, chunks=dict(time=-1)).squeeze()
            dd = dd.rename({'time':'leadtime'})
            dd['leadtime'] = np.arange(len(dd['leadtime']))
            dd['time'] = pd.to_datetime(r.split("/")[-1].replace("_SST.nc",""))
            data.append(dd)
        data = xr.concat(data,'time')
        data.to_netcdf(p+'/merged_SST.nc')
    else:
        data = xr.open_dataset(p+'/merged_SST.nc').sortby('lat').sel(lat=slice(-60,15), lon=slice(360-150,360-69))
        data = data.load()
    DATA.append(data)
print('Joining...')
...
DATA = DATA['sst'].drop_duplicates('time').resample({'time':'d'}).interpolate(kind='nearest')

print(DATA)

print('computing climatology')
for i,lead in enumerate(DATA.leadtime):
        clim = climatology_xarray(DATA.sel(leadtime=lead), 'time')
        #anom = DATA.sel(leadtime=lead).dslm.convert_calendar('noleap').groupby('time.dayofyear')-clim
        print('saving to netcdf: LEADTIME '+str(i))
#for i,lead in enumerate(clim.leadtime):
        clim.to_dataset(name='sst').to_netcdf('data/S2S/CLIMATOLOGY/SST_LEADTIME'+str(i)+'.nc',
                                               encoding={'sst':{'dtype':'float32'}})
       # anom.to_dataset(name='zos_anomaly').to_netcdf('data/S2S/reforecasts/i')    
